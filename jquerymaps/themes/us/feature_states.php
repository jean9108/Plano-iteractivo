<?php
	...
	$sql = "SELECT r.rangeID, r.range_min, r.range_max FROM maps_ranges r WHERE r.mapID = 'us' ORDER BY r.rangeID ";
	...
	if ($query->num_rows > 0)
		while ($rd = $query->fetch_assoc()) {
			...
			echo "<feature id=\"" . strtolower($rd['countryID'] . "_" . $rd['stateID']) . "\" category=\"state_" . $category . "\" label=\"" . $rd['state'] . "\" label_map=\"" . strtoupper($rd['stateID']) . "\" popup=\"" . $year . " Population: " . number_format($rd['total'], 0) . "\" y=\"" . $year . "\" map=\"us\" />\n";
		}
	$query->close();
	$jqm_db->close();
