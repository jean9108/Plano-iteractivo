<?php
	...
	//*** COUNTIES ***	
	//**************
	$sql = "SELECT c.countryID, c.stateID, c.countyID, c.county, c.population_" . $year . " AS total ";
	...
	if ($query->num_rows > 0)
		while ($rd = $query->fetch_assoc()) {
			...
			echo "<feature id=\"" . strtolower($rd['countryID'] . "_" . $rd['stateID'] . "_" . $rd['countyID']) . "\" category=\"county_" . $category . "\" label=\"" . htmlentities($rd['county']) . "\" popup=\"" . $year . " Population: " . number_format($rd['total'], 0) . "\" y=\"" . $year . "\" map=\"uscounties\" />\n";
		}
	$query->close();
	$jqm_db->close();
